...
![image-20260403134223503](./images/README/image-20260403134223503.png)
...
* **错题本系统**：用户可将答错或薄弱的考题一键收藏，随时回顾考官评语与核心知识点，构建个人弱点库。
...
3. 运行编译器（每份笔记默认裂变 3 个变种靶机）：

```cmd
python build.py
```

> 可在 `build.py` 顶部配置区调整 `DERIVE_COUNT`（衍生倍率）和 `TARGET_LABS`（指定编译的笔记 ID）。
